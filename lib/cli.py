...
class FinanceTrackerCLI:
    ...
    def print_header(self):
        ...
        print(
            f"{Back.BLUE}{Fore.WHITE}{'💰 PERSONAL FINANCE TRACKER':^60}{Style.RESET_ALL}")
        print(f"{Back.BLUE}{Fore.WHITE}{'=' * 60}{Style.RESET_ALL}")
        print(
            f"{Fore.GREEN}👤 Current User: {self.user_name} (ID: {self.user_id}){Style.RESET_ALL}\n")
    ...
    def get_user_input(self, prompt, input_type=str, validation_func=None):
        ...
        while True:
            try:
                user_input = input(
                    f"{Fore.CYAN}{prompt}{Style.RESET_ALL}").strip()
                ...
                if validation_func and not validation_func(value):
                    print(
                        f"{Fore.RED}✗ Invalid input. Please try again.{Style.RESET_ALL}")
                    continue

                return value
            except ValueError:
                print(
                    f"{Fore.RED}✗ Please enter a valid {input_type.__name__}.{Style.RESET_ALL}")
    ...
    def select_or_create_user(self):
        ...
        if choice is None:
            # Default to first user if cancelled
            self.user_id = users[0]['user_id']
            self.user_name = users[0]['name']
            return
    ...
    def create_new_user(self):  # Create a new user
        ...
        try:
            ...
            conn.close()
            print(
                f"{Fore.GREEN}✓ User '{self.user_name}' created successfully!{Style.RESET_ALL}")
        except Exception as e:
    ...
    def user_management(self):
        ...
        choice = self.get_user_input(
            "Choose option (1-4): ", int, lambda x: 1 <= x <= 4)
    ...
    def switch_user(self):  # Switch to different user
        ...
        if selected_user['user_id'] != self.user_id:
            ...
            print(
                f"{Fore.YELLOW}Already logged in as {self.user_name}{Style.RESET_ALL}")
    ...
    def view_all_users(self):  # Display all users
        ...
        for user in users:
            ...
            print(
                f"{user['user_id']:<4} {user['name']:<20} {status_color}{status}{Style.RESET_ALL}")
    ...
    def view_transactions(self):  # View transactions with filtering options
        ...
        choice = self.get_user_input(
            "Choose filter (1-4): ", int, lambda x: 1 <= x <= 4)
        if choice is None:
            return
        ...
        if choice == 1:
            ...
            month = self.get_user_input(
                "Month (1-12): ", int, lambda x: 1 <= x <= 12)
            if month is None:
                return
    ...
    def display_transactions(self, transactions):
        ...
        print("─" * 80)
        print(
            f"{'ID':<4} {'Date':<12} {'Category':<15} {'Amount':<12} {'Description':<25}")
        print("─" * 80)
        ...
        print("─" * 80)
        print(
            f"Total Income: {Fore.GREEN}+${total_income:.2f}{Style.RESET_ALL}")
        print(
            f"Total Expenses: {Fore.RED}-${total_expenses:.2f}{Style.RESET_ALL}")
        print(
            f"Net: {Fore.CYAN}${total_income - total_expenses:.2f}{Style.RESET_ALL}")
    ...
    def view_balance(self):
        ...
        if balance >= 0:
            print(
                f"Current Balance: {Fore.GREEN}${balance:.2f}{Style.RESET_ALL}")
        else:
            print(
                f"Current Balance: {Fore.RED}${balance:.2f}{Style.RESET_ALL}")
    ...
    def manage_budgets(self):
        ...
        choice = self.get_user_input(
            "Choose option (1-3): ", int, lambda x: 1 <= x <= 3)
        if choice is None:
            return
    ...
    def update_budget(self):
        ...
        for item in summary:
            if item['limit']:
                print(f"- {item['category']}: ${item['limit']:.2f}")

        category = self.get_user_input("\nCategory to update: ")
        if not category:
            return

        new_amount = self.get_user_input(
            "New budget limit: $", float, lambda x: x > 0)
    ...
    def view_budget_summary(self):  # Display budget summary
        ...
        print(
            f"{'Category':<15} {'Limit':<12} {'Spent':<12} {'Remaining':<12} {'Status':<10}")
        print("─" * 70)

        for item in summary:
            category = item['category']
            limit = item['limit']
            spent = abs(item['spent'])  # Make spending positive for display
            status = item['status']
    ...
    def delete_transaction(self):  # Delete a transaction
        ...
        if not transaction:
            print(
                f"{Fore.RED}✗ Transaction not found in recent transactions.{Style.RESET_ALL}")
            input(f"\n{Fore.CYAN}Press Enter to continue...{Style.RESET_ALL}")
            return
    ...
    def run(self):  # Main application loop with updated menu order
        while True:
            ...
            choice = self.get_user_input(
                "Select an option (1-10): ", int, lambda x: 1 <= x <= 10)
            ...
            try:
                if choice == 1:
                    self.user_management()  # User Management is now first
                elif choice == 2:
